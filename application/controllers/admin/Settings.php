...
class Settings extends My_controller {
	...
	public function general()
	{
		...
		if($this->input->post()){
            ...
            if(file_exists($_FILES['site_logo']['tmp_name']) && is_uploaded_file($_FILES['site_logo']['tmp_name'])){
             ...
             if(file_exists($_FILES['site_logo_bottom']['tmp_name']) && is_uploaded_file($_FILES['site_logo_bottom']['tmp_name'])){
                $imt = 'logo_bottom';
            	if($_FILES['site_logo_bottom']["type"] == "image/jpeg" || $_FILES['site_logo_bottom']["type"] == "image/JPEG"){

                    $img = $imt.'_'.$timestamp . '.jpeg';
                
                }else if($_FILES['site_logo_bottom']["type"] == "image/png" || $_FILES['site_logo_bottom']["type"] == "image/PNG"){
                
                    $img = $imt.'_'.$timestamp . '.png';
                
                }else if($_FILES['site_logo_bottom']["type"] == "image/jpg" || $_FILES['site_logo_bottom']["type"] == "image/JPG"){
            
                    $img = $imt.'_'.$timestamp . '.jpg';
                    
                }else if($_FILES['site_logo_bottom']["type"] == "image/gif" || $_FILES['site_logo_bottom']["type"] == "image/GIF"){
            	...
            	$target = "uploads/settings/"; 
                $target = $target . basename($img); 
                move_uploaded_file($_FILES['site_logo_bottom']['tmp_name'], $target);
                $data['site_logo_bottom'] = $img;
            }
	...
	public function edit_zone($id, $city_id)
	{
	    if (!$this->auth->is_logged()) {
            redirect('admin/authentication');
            exit();
		...
		if($this->input->post()){
		    ...
		    $data['name']=$name;
		    $this->db->where('id', $id);
		    $this->db->update('app_zones', $data);
		    $this->set_message('success', "Zone updated successfully.");
    		redirect('admin/settings/zones?city_id='.$city_id);
            exit();
		    
		}
		
		$this->title = "Edit Zone || ".$this->admintitle;
	   ...
	   // $data['checkzone'] =  $this->db->query("SELECT * FROM app_zones where name = '$name' && city_id = '$city_id'");
	   // print_r ($data['getcities']);exit;
	   
	    $data['cities'] = $this->db->select("*")->from('app_cities')->order_by('name', 'ASC')->get()->result_array();
	    $data['city_id'] = $city_id;
	    $data['zone'] =  $this->db->select("*")->from('app_zones')->where('id', $id)->get()->row_array();
		
        $this->load_admin('settings/edit_zone',$data);
	}
	...
	public function edit_area($id, $zone_id)
	{
	    if (!$this->auth->is_logged()) {
            redirect('admin/authentication');
            exit();
		...
		if($this->input->post()){
		    ...
		    $this->db->update('app_areas', $data);
		    $this->set_message('success', "Area updated successfully.");
    		redirect('admin/settings/areas?zone_id='.$zone_id);
            exit();
		    
		}
		
		$this->title = "Edit Area || ".$this->admintitle;
...
// 		$data['cities'] = $this->db->select("*")->from('app_cities')->order_by('name', 'ASC')->get()->result_array();
	    $data['city_id'] = $zone_id;
	    $data['zones'] =  $this->db->select("*")->from('app_areas')->where('zone_id', $id)->get()->row_array();
	    
	    $this->db->select("*")->from('app_zones')->order_by('name', 'ASC');
		
		if($this->input->get('zone_id')){
		    $city_id = $this->db->query("SELECT * FROM app_zones where id = '{$_GET['zone_id']}'")->row_array();
		}
		
		$data['cities'] = $this->db->get()->result_array();
		
        $this->load_admin('settings/edit_area',$data);
	}
	
	public function delete_area($id, $city_id){
	    
	    $check_address = $this->db->query("SELECT * FROM `app_address` where area_id = '$id'")->num_rows();
		
		   
		if($check_address > 0){
		    $this->set_message('error', "Address is already added within this zone.To delete this please delete the address first.");
			redirect('admin/settings/areas?zone_id='.$zone_id);
			exit();
		}else{
	    
	    $this->db->where('id', $id);
	    $this->db->delete('app_areas');
	    $this->set_message('success', "Area deleted successfully");
	    redirect('admin/settings/areas?zone_id='.$zone_id);
        exit();
	}
	}
	
		public function delete_zone($id, $city_id){
	    
	    $check_address = $this->db->query("SELECT * FROM `app_areas` where zone_id = '$id'")->num_rows();
		
		   
		if($check_address > 0){
		    $this->set_message('error', "We already have cutomers in this Zone.");
			redirect('admin/settings/zones?city_id='.$city_id);
			exit();
		}else{
	    
	    $this->db->where('id', $id);
	    $this->db->delete('app_zones');
	    $this->set_message('success', "Zone deleted successfully");
	    redirect('admin/settings/zones?city_id='.$city_id);
        exit();
	}
	}
