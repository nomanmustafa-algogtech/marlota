...
class Products extends My_controller {
	...
	public function index()
	{
		...
		if($this->input->post()){
		   ...
		   foreach($product_ids as $product_id){
    		   ...
    		   $this->db->where(array('id'=>$product_id, 'vendor_id'=>$_SESSION['vendor_id']));
	    ...
	    $data['products'] = $this->db->query("SELECT p.id, s.sku, s.price, p.featured, p.thumbnail_img, p.published, p.category_id, c.name FROM app_product_stocks s, app_products p, app_categories c WHERE p.id = s.product_id && c.id = p.category_id && p.vendor_id = '{$_SESSION['vendor_id']}' Order by p.id asc")->result_array();
		
        $this->load_vendor('products/products',$data);
	}
	...
	public function attributes()
	{
		...
		if($this->input->post()){
		    ...
		    $check_name = $this->db->select('*')->from('app_attributes')->where(array('name', $data['name'], 'vendor_id'=>$_SESSION['vendor_id']))->get()->num_rows();
		    if($check_name > 0){
		        $this->set_message('error', "Attribute name already exists.");
    		    redirect('vendors/products/attributes');
            	exit();
		    }
	    ...
	    $data['attributes'] = $this->db->select("*")->from('app_attributes')->where(array('vendor_id'=>$_SESSION['vendor_id']))->order_by('name', 'ASC')->get()->result_array();
		
        $this->load_vendor('products/attributes',$data);
	}
	...
	public function attribute_delete($type, $id){
	    ...
	    if($type == 'name'){
		    ...
		    redirect('vendors/products/attribute_values/'.$_GET['att_id']);
        	exit();
	    }
	}
	...
	public function attribute_edit($type, $id){
	    ...
	    if($type == 'name'){
	        ...
	        if($this->input->post()){
	            ...
	            $check_name = $this->db->select('*')->from('app_attributes')->where(array('name'=>$data['name'], 'id!='=>$id, 'vendor_id'=>$_SESSION['vendor_id']))->get()->num_rows();
    		    if($check_name > 0){
    		        $this->set_message('error', "Attribute name already exists.");
        		    redirect("vendors/products/attribute_edit/$type/$id");
                	exit();
    		    }
    		    $this->db->where(array('id'=>$id, 'vendor_id'=>$_SESSION['vendor_id']));
    	    ...
    	    $data['attribute'] = $this->db->select("*")->from('app_attributes')->where(array('id'=>$id, 'vendor_id'=>$_SESSION['vendor_id']))->order_by('name', 'ASC')->get()->row_array();
    		
            $this->load_vendor('products/edit_attribute',$data);
	        ...
	        if($this->input->post()){
    		    ...
    		    redirect('vendors/products/attribute_values/'.$attribute_id);
            	exit();
	        }
    	    ...
    	    $data['attribute'] = $this->db->select("*")->from('app_attributes')->where(array('id'=>$data['attribute_value']['attribute_id'], 'vendor_id'=>$_SESSION['vendor_id']))->order_by('name', 'ASC')->get()->row_array();
    		
            $this->load_vendor('products/edit_attribute_value',$data);
	    }
	}
	...
	public function attribute_values($id = 0){
	    ...
	    $attribute = $this->db->select("*")->from('app_attributes')->where(array('id'=>$id, 'vendor_id'=>$_SESSION['vendor_id']))->get();
	    ...
	    if($this->input->post()){
		    ...
		    if($check_value > 0){
		        $this->set_message('error', "Attribute value already exists.");
    		    redirect('vendors/products/attribute_values/'.$id);
            	exit();
		    }
        ...
        $this->load_vendor('products/attribute_values',$data);
...
// 	}
	...
	public function add_new()
	{
		...
		if($this->input->post()){
		    ...
		    if($check_slug > 0){
    		    ...
    		    redirect('vendors/products/add_new');
            	exit();
		    }
		    
		    $dataProduct['vendor_id'] = $this->session->userdata('vendor_id');
		    $dataProduct['user_id'] = $this->session->userdata('vendor_id');
		    ...
		    if(file_exists($_FILES['thumbnail']['tmp_name']) && is_uploaded_file($_FILES['thumbnail']['tmp_name'])){
            	...
            	if($img_uploaded){
            	    ...
            	    $this->set_message('error', "Thumbnail Image is invalid");
            			redirect('vendors/products/add_new');
            		exit();
            	}
            }
	    ...
	    $data['brands'] = $this->db->select("*")->from('app_brands')->order_by('name', 'ASC')->get()->result_array();
	    $data['suppliers'] = $this->db->select("*")->from('app_suppliers')->order_by('name', 'ASC')->get()->result_array();
	    $data['attributes'] = $this->db->select("*")->from('app_attributes')->where('vendor_id', $_SESSION['vendor_id'])->order_by('name', 'ASC')->get()->result_array();
		
        $this->load_vendor('products/add_new',$data);
	}
	...
	public function edit($id)
	{
	   if (!$this->session->userdata('vendor_loggedin')) {
		...
		$product = $this->db->select("*")->from('app_products')->where(array('id'=> $id, 'vendor_id'=> $_SESSION['vendor_id']))->get();
...
// 		exit;
		...
		if($this->input->post()){
		    ...
		    if($check_slug > 0){
		        $this->set_message('error', "Slug already Exists.");
    		    redirect('vendors/products/edit/'.$id);
            	exit();
		    }
        ...
        $this->load_vendor('products/edit',$data);
	}
