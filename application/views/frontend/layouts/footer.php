...
<section class="product footer">
                        ...
                        <div class="col-lg-3 col-sm-6">
                            <div class="service-wrapper free-shipping">
                                ...
                                <div class="service-content">
                                    ...
                                    <p class="service-info service-details">Fast & Free Shipping, Every Order, Every Time</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-3 col-sm-6">
                    ...
                    <div class="row gy-5">
                        <div class="col-lg-3 col-sm-6">
                            <div class="footer-order">
                                <div class="logo">
                                     <img width="200" src="<?php echo base_url(); ?>uploads/settings/<?php echo $this->settings['site_logo']; ?>" alt="<?php echo $settings['site_title']; ?>" />
                                </div>
                                ...
                                <div class="footer-link contact-link">
                                    <div class="address">
                                        ...
                                        <div class="details">
                                            ...
                                            <p>Marlota Ltd Unit 1 Bealey Industrial Estate Radcliffe Bury M26 2BD.</p>
                                        </div>
                                    </div>
		...
		<div class="toast-container position-fixed bottom-0 start-0 p-3" style="z-index: 1100">
			<div id="cartToast" class="toast align-items-center text-bg-light border-0 shadow-lg" role="alert" aria-live="assertive" aria-atomic="true" style="min-width: 350px;">
				<div class="d-flex">
				<div class="toast-body w-100">
					<div class="d-flex align-items-center">
					<img id="toastProductImg" src="" alt="Product" class="me-3 rounded border" width="70" height="70">
					<div>
						<strong id="toastProductName" class="d-block mb-1"></strong>
						<small class="text-success">✅ Added to cart successfully!</small>
					</div>
					</div>
					<div class="mt-3 d-flex gap-2">
					<a href="<?= base_url('cart'); ?>" class="btn btn-sm btn-outline-primary flex-fill">View Cart</a>
					<a href="<?= base_url('checkout'); ?>" class="btn btn-sm btn-primary flex-fill">Checkout</a>
					</div>
				</div>
	...
	?>
<script>
	function productRoot(btn) {
		return btn.closest(".product, .product-popup, .home .product-single");
	}

	$(document).on("click", ".product:not(.product-select) .btn-cart, .product-popup .btn-cart, .home .product-single .btn-cart", function(e) {
    e.preventDefault();
    ...
    setTimeout(function() {
        var product_id = $("#product_id").val();
		  var root   = productRoot(i);
         var qtyBox = root.find(".quantity").first();
         var qty    = parseInt(qtyBox.find(".number").text(), 10); if (isNaN(qty) || qty < 1) qty = 1;

        // ✅ Parse price safely
        var price = parseFloat($("#product-price .new-price").text().replace(/[^\d.]/g, '')) || 0;
        var cart_old_total = parseFloat($(".cart-total .price").text().replace(/[^\d.]/g, '')) || 0;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data == 'ERROR_QTY') {
                    ...
                    i.removeClass("load-more-overlay loading added");
                } else {
					$("#toastProductImg").attr("src", a.find(".product-media img, .product-image:first-child img").attr("src"));
					$("#toastProductName").text(a.find(".product-name, .product-title").text());
                    ...
                    var oldcart = parseInt($(".cart-counter").html()) || 0;
                    $(".cart-counter").html(oldcart + 1);

                    var new_total = cart_old_total + (price * qty);
                    $(".cart-total .price").html("£ " + new_total.toFixed(2));

                    $(".cart-dropdown .products").append(`
                        <div class="product product-cart">
                            <div class="product-detail">
                                <a href="<?=base_url();?>products/view/${$("#product-slug").val()}" class="product-name">
                                    ${a.find(".product-name, .product-title").text()}
                                </a>
                                <div class="price-box">
                            ...
                            <figure class="product-media">
                                <a href="<?=base_url();?>products/view/${$("#product-slug").val()}">
                                    <img src="${a.find(".product-media img, .product-image:first-child img").attr("src")}" 
                                         alt="${a.find(".product-name, .product-title").text()}" height="84" width="94" />
                                </a>
                            </figure>
                        </div>
                    `);
  ...
  $(document).on("click", ".quantity .plus, .quantity .minus", function () {
    ...
    var numEl  = box.find(".number").first();
    var current = parseInt(numEl.text(), 10); if (isNaN(current)) current = 1;

    var min = parseInt(box.data("min"), 10); if (isNaN(min)) min = 1;
    var max = parseInt(box.data("max"), 10); if (isNaN(max)) max = 1000;
